...
is in `config.py`, never at a call site. A run above `$1.50` warns, because
the only two ways to get there are a missed cache or a zoom stage firing on
most sheets — both real problems.

Every stage's model is overridable from config, the environment
(`SCHEME_MODEL_AUDIT=claude-sonnet-5`) and the request body. Upgrading the
audit stage is a one-line change, which is the point: if Haiku fails the
merged-span or cross-sheet gates, upgrade stage 7 alone first.
